(function() {
  ...
  window._deleteCurrent = async function() {
    ...
    try {
      var deleteBody = { filename: filename };
      if (currentDuplicates.length > 0) deleteBody.hadDuplicate = true;
  ...
  window._moveTo = async function(folder) {
    ...
    try {
      ...
      // Choice tracking: determine how the folder was chosen
      if (showingFuzzy) {
        body.wasManual = true;
        body.chosenRank = null;
      } else {
        body.wasManual = false;
        var matchIdx = currentResults.findIndex(function(r) { return r.folder === folder; });
        body.chosenRank = matchIdx >= 0 ? matchIdx + 1 : null;
      }
      // Per-method ranks and score for the chosen folder
      var chosenResult = currentResults.find(function(r) { return r.folder === folder; });
      if (chosenResult) {
      ...
      // Duplicate tracking
      if (currentDuplicates.length > 0) body.hadDuplicate = true;
  ...
  window._skip = function() {
    ...
    if (currentIdx < pdfs.length - 1) {
      if (!isHandled(pdfs[currentIdx])) {
        ...
        var skipBody = { action: 'skip', filename: pdfs[currentIdx] };
        if (currentDuplicates.length > 0) skipBody.hadDuplicate = true;
        fetch('/api/log', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
