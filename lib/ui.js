...
export function startUiServer(targetFolder, { port }) {
  // Load model once at startup (let so background reindex can refresh them)
  let centroids = getAllCentroids();
  const bayesJson = loadBayesState();
  let bayes = NaiveBayes.deserialize(bayesJson);
  ...
  centroids.forEach(c => diskFolders.add(c.folder));
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      // Static SPA assets
      if (req.method === 'GET' && (path === '/' || path === '/style.css' || path.endsWith('.js'))) {
        let file, type;
        if (path === '/') {
          file = 'index.html';
          ...
          // Serve .js files with path traversal guard
          file = path.slice(1); // strip leading /
