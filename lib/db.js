...
export function insertDoc(path, folder, text, embedding) {
  const db = getDb();
  const buf = Buffer.from(embedding.buffer, embedding.byteOffset, embedding.byteLength);
  db.prepare(
    'INSERT OR REPLACE INTO documents (path, folder, text, embedding) VALUES (?, ?, ?, ?)'
  ).run(path, folder, text, buf);
}
...
export function getAllDocs() {
  const db = getDb();
  const rows = db.prepare('SELECT id, path, folder, text, embedding FROM documents').all();
  return rows.map(r => ({
    ...r,
...
export function getDocsByFolder(folder) {
  const db = getDb();
  const rows = db.prepare('SELECT id, path, folder, text, embedding FROM documents WHERE folder = ?').all(folder);
  return rows.map(r => ({
    ...r,
    embedding: new Float64Array(r.embedding.buffer, r.embedding.byteOffset, r.embedding.byteLength / 8),
  }));
...
export function clearModel() {
  const db = getDb();
  db.exec('DELETE FROM documents; DELETE FROM centroids; DELETE FROM bayes_state;');
}
