...
export function insertStat(event, data) {
  const db = getDb();
  db.prepare('INSERT INTO stats (event, data) VALUES (?, ?)').run(event, JSON.stringify(data));
}

export function getStats() {
  const db = getDb();
  return db.prepare('SELECT id, event, data, created_at FROM stats ORDER BY id').all()
    .map(r => ({ ...r, data: r.data ? JSON.parse(r.data) : null }));
}

export function getStatsByEvent(event) {
  const db = getDb();
  return db.prepare('SELECT id, event, data, created_at FROM stats WHERE event = ? ORDER BY id').all(event)
    .map(r => ({ ...r, data: r.data ? JSON.parse(r.data) : null }));
}
...
/**
 * Throws if stored embed model differs from currentModel.
 * @param {string} currentModel
 */
export function assertModelCompatible(currentModel) {
  const storedModel = getMeta('embed_model');
  if (storedModel && storedModel !== currentModel) {
    throw new Error(
      `Data was indexed with "${storedModel}" but current model is "${currentModel}". ` +
      'Run `docc reset` then `docc learn` to re-index with the new model.'
    );
  }
}
